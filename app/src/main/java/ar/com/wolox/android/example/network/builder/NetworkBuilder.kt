...
class NetworkBuilder<T> {
    ...
    fun build(networkResponse: NetworkResponse<Response<T>>) {
        return when (networkResponse) {
            ...
            is NetworkResponse.Error -> onResponseFailed.invoke(networkResponse.response.body(), networkResponse.response.code())
